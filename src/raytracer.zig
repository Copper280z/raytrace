...
pub noinline fn hittableList(T: anytype) type {
    return struct {
        objects: T,
...
pub fn build_BVH(parent_arr: *std.ArrayList(Sphere), obj_inds: []u32, depth: u32, allocator: std.mem.Allocator, tmp_allocator: std.mem.Allocator) *TreeNode {
    BVHNodes += 1;
    var objects = std.ArrayList(obj_ind).init(tmp_allocator);
    ...
    if ((depth == 0) or (obj_inds.len <= 3)) {
        ...
        const inds = allocator.alloc(u32, obj_inds.len) catch unreachable;
        errdefer allocator.free(inds);
        std.mem.copyForwards(u32, inds, obj_inds);
    ...
    var sorted_inds = std.ArrayList(u32).init(allocator);
    ...
    node.bbox = new_bbox;
    node.child1 = build_BVH(parent_arr, sorted_inds.items[0..split], depth - 1, allocator, tmp_allocator);
    node.child2 = build_BVH(parent_arr, sorted_inds.items[split..], depth - 1, allocator, tmp_allocator);
...
pub fn Random_Scene(half_size: usize, rng: std.Random, allocator: std.mem.Allocator, tmp_allocator: std.mem.Allocator) !Hittable {
    ...
    std.debug.print("World has {} objects\n", .{list.items.len});
    var World: Hittable = undefined;

    if (Hittable == HittableBVH([]const Sphere)) {
        var inds = std.ArrayList(u32).init(allocator);
        defer inds.deinit();

        for (0..list.items.len) |i| {
            try inds.append(@intCast(i));
        }

        const BVH = build_BVH(list, inds.items, 5, allocator, tmp_allocator);
...
pub var cam = cam_types.MovableCamera{};
...
pub fn main() !void {
    ...
    std.debug.print("Leaf Node is {} bytes\n", .{@sizeOf(LeafNode)});
    const filetype = .ppm;
    ...
    const World = try Random_Scene(11, rng.random(), arena.allocator(), fba.allocator());
    std.debug.print("World is {} bytes\n", .{arena.queryCapacity()});
